...
type IMolecule = InstanceType<typeof Molecule>;
...
function getCoordinates(mol: IMolecule, atom: number) {
  return {
    x: mol.getAtomX(atom),
    y: mol.getAtomY(atom),
    z: mol.getAtomZ(atom),
...
function getAllCoordinates(mol: IMolecule) {
  const coordinates = [];
...
// TODO(wasm): different seeds produce identical coordinates under WasmGC (the
// random seed does not vary the layout). Needs investigation.
test.skip('custom seeds', () => {
