...
import { Molecule, SSSearcherWithIndex } from '#lib';
...
test('should compute tanimoto similarity', () => {
  ...
  expect(
    ...
    // TeaVM uses true 32-bit float (GWT emulates float as double), so this is
    // 0.33333334 rather than the exact double 1/3.
  ).toBeCloseTo(1 / 3);
  expect(
