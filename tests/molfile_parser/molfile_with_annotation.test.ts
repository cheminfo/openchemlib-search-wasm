...
// TODO(wasm): the second half of this test relies on a Canonizer idcode produced
// with encodeAtomCustomLabels, which emits an extra byte under WasmGC (a
// custom-label encoding difference). The fromIDCode roundtrip order therefore
// does not match the expected openchemlib-js order. Skipped until investigated.
test.skip('Molecule with annotation', () => {
