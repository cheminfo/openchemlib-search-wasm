...
import { Canonizer, CanonizerUtil, Molecule } from '#lib';
...
test('getFinalRank returns an array with expected length', () => {
  ...
  expect(finalRank).toBeInstanceOf(Int32Array);
...
test('getSymmetryRank and getSymmetryRanks return consistent results', () => {
  ...
  expect(symmetryRanks).toBeInstanceOf(Int32Array);
...
test('setParities, getGraphAtoms, and getGraphIndexes produce consistent results', () => {
  ...
  expect(graphAtomsBefore).toBeInstanceOf(Int32Array);
  expect(graphIndexesBefore).toBeInstanceOf(Int32Array);
  expect(graphAtomsAfter).toBeInstanceOf(Int32Array);
  expect(graphIndexesAfter).toBeInstanceOf(Int32Array);
  expect(graphAtomsAfter).toHaveLength(graphAtomsBefore.length);
  expect(graphIndexesAfter).toHaveLength(graphIndexesBefore.length);
