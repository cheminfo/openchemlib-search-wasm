...
public class StatisticalTorsionPotential {
	...
	private StatisticalTorsionPotential() {
		...
		if (database == null) {
			InputStream is = FakeFileInputStream.getResourceAsStream(BASE_PATH+DATABASE_CSD+TORSION_BINS_FILE);
			if (is != null) {
	...
	private void initialize() {
		BufferedReader torsionIDReader = new BufferedReader(new InputStreamReader(FakeFileInputStream.getResourceAsStream(
